...
def calc_stats(list_of_clients):
...
def analyse_pa1(pa_dict, cs):
    ...
    pa_dict['PA1 % with symptoms'] = (
        "{:.2f}".format(cs.ratio_of_matches(lambda client: client.has_key_symptom_at_i('medical_score', 0))))
    ...
    pa_dict['PA1 % prolapse'] = "{:.2f}".format(cs.ratio_of_matches(
        lambda client: client.has_key_symptom_at_i('medical_prolapse_4'), len(cs.list_of_clients)))
    pa_dict['PA1 % stage 1'] = "{:.2f}".format(cs.ratio_of_matches(
        lambda client: client.quests[0]['medical_prolapse_4'] == 1, len(cs.list_of_clients)))
    pa_dict['PA1 % stage 2'] = "{:.2f}".format(cs.ratio_of_matches(
        lambda client: client.quests[0]['medical_prolapse_4'] == 2, len(cs.list_of_clients)))
    pa_dict['PA1 % stage 3'] = "{:.2f}".format(cs.ratio_of_matches(
        lambda client: client.quests[0]['medical_prolapse_4'] == 3, len(cs.list_of_clients)))
    pa_dict['PA1 % stage 4'] = "{:.2f}".format(cs.ratio_of_matches(
        lambda client: client.quests[0]['medical_prolapse_4'] == 4, len(cs.list_of_clients)))
    ...
    pa_dict['PA1 % no birth'] = pa_dict['PA1 % birth'] = "{:.2f}".format(cs.ratio_of_matches(
    ...
    pa_dict['PA1 % premenopausal'] = "{:.2f}".format(cs.ratio_of_matches(
        lambda client: client.quests[0]['menstrual_status'] == 'pre_menopausal', len(cs.list_of_clients)))
    pa_dict['PA1 % postmenopausal'] = "{:.2f}".format(cs.ratio_of_matches(
        lambda client: client.quests[0]['menstrual_status'] == 'post_menopausal', len(cs.list_of_clients)))
    ...
    pa_dict['PA1 % subjective excellent'] = "{:.2f}".format(cs.ratio_of_matches(
        lambda client: client.quests[0]['subjective'] == 0, len(cs.list_of_clients)))
    pa_dict['PA1 % subjective good'] = "{:.2f}".format(cs.ratio_of_matches(
        lambda client: client.quests[0]['subjective'] == 1, len(cs.list_of_clients)))
    pa_dict['PA1 % subjective average'] = "{:.2f}".format(cs.ratio_of_matches(
        lambda client: client.quests[0]['subjective'] == 2, len(cs.list_of_clients)))
    pa_dict['PA1 % subjective poor'] = "{:.2f}".format(cs.ratio_of_matches(
        lambda client: client.quests[0]['subjective'] == 3, len(cs.list_of_clients)))
    pa_dict['PA1 % subjective very poor'] = "{:.2f}".format(cs.ratio_of_matches(
        lambda client: client.quests[0]['subjective'] == 4, len(cs.list_of_clients)))
    pa_dict['PA1 mean sexual wellbeing'] = "{:.2f}".format(cs.mean_of_vals(
        lambda client: client.get_num_key_at_i('sex_score', 0)))
...
def analyse_non_specific_pa(i, cs, pa_dict):
    ...
    pa_dict[f'pa{i + 1} # with UI symptoms at PA1'] = cs.count_matches(
        lambda client: client.has_not_key_symptom_at_i('medical_score', i))
    pa_dict[f'pa{i + 1} % with UI symptoms at PA1'] = "{:.2f}".format(cs.ratio_of_matches(
        lambda client: client.has_key_symptom_at_i('medical_score', 0), pa_dict[f'pa{i + 1} # respondents']))
    pa_dict[f'pa{i + 1} # no UI symptoms at PA1'] = cs.count_matches(
        lambda client: client.has_not_key_symptom_at_i('medical_score', 0))
    pa_dict[f'pa{i + 1} % no UI symptoms at PA1'] = "{:.2f}".format(cs.ratio_of_matches(
        lambda client: client.has_not_key_symptom_at_i('medical_score', 0), pa_dict[f'pa{i + 1} # respondents']))
    ...
    pa_dict[f'pa{i + 1} UI % better subjective'] = ":.2f".format(cs.ratio_of_matches(
        ...
        cs.count_matches(lambda client: client.has_key_symptom_at_i('medical_score'))))
    ...
    pa_dict[f'pa{i + 1} UI % better sexual wellbeing'] = ":.2f".format(cs.ratio_of_matches(
        lambda client: client.has_key_symptom_at_i('medical_score', 0) and client.got_better_at_i('sex_score', i),
        cs.count_matches(lambda client: client.has_key_symptom_at_i('medical_score'))))
    pa_dict[f'pa{i + 1} UI % same sexual wellbeing'] = ":.2f".format(cs.ratio_of_matches(
        lambda client: client.has_key_symptom_at_i('medical_score', 0) and client.got_no_change_at_i('sex_score', i),
        cs.count_matches(lambda client: client.has_key_symptom_at_i('medical_score'))))
    pa_dict[f'pa{i + 1} UI % worse sexual wellbeing'] = ":.2f".format(cs.ratio_of_matches(
        lambda client: client.has_key_symptom_at_i('medical_score', 0) and client.got_worse_at_i('sex_score', i),
        cs.count_matches(lambda client: client.has_key_symptom_at_i('medical_score'))))
    pa_dict[f'pa{i + 1} # with prolapse at PA1'] = cs.count_matches(
        lambda client: client.has_key_symptom_at_i('medical_prolapse_4', 0))
    ...
    pa_dict[f'pa{i + 1} % better prolapse symptoms'] = ":.2f".format(cs.ratio_of_matches(
        lambda client: client.got_better_at_i('medical_prolapse_symptoms', i),
        pa_dict[f'pa{i + 1} # with prolapse at PA1']))
    pa_dict[f'pa{i + 1} % same prolapse symptoms'] = ":.2f".format(cs.ratio_of_matches(
        lambda client: client.got_no_change_at_i('medical_prolapse_symptoms', i),
        pa_dict[f'pa{i + 1} # with prolapse at PA1']))
    pa_dict[f'pa{i + 1} % worse prolapse symptoms'] = ":.2f".format(cs.ratio_of_matches(
        lambda client: client.got_worse_at_i('medical_prolapse_symptoms', i),
        pa_dict[f'pa{i + 1} # with prolapse at PA1']))
    ...
    pa_dict[f'pa{i + 1} mean time since PA1 NA'] = ":.2f".format(cs.mean_of_vals(
        ...
        if client.has_not_key_symptom_at_i('medical_score', 0) and client.has_not_key_symptom_at_i('medical_prolapse_4')
        else None
    ))
    ...
    pa_dict[f'pa{i + 1} NA % better subjective'] = ":.2f".format(cs.ratio_of_matches(
        lambda client: client.got_better_at_i('subjective', i)
        if client.has_not_key_symptom_at_i('medical_score', 0) and client.has_not_key_symptom_at_i('medical_prolapse_4')
        else None, pa_dict[f'pa{i + 1} # with no prolapse no UI at PA1']
    ))
