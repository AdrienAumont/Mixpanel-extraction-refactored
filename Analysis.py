...
def calc_stats(list_of_clients: list[Client]):
    """
    the main function of the class that innitiates the calculation of the statistics for every wave of assessment
    :param list_of_clients: the list of clients to analyse
    :return: dict(str : Number) a dictionary to export to csv
    """
...
def analyse_pa1(pa_dict, cs):
    """
    calculate the stats for the first wave of assessment that is quite different from the next waves
...
def analyse_non_specific_pa(i, cs, pa_dict):
    """
    calculate the stats for the ith wave of assessment
    :param i: the number of the wave
    :param cs: the ClientStatistics object to perform the calculations
    :param pa_dict: the dict to update passed by reference
    """
